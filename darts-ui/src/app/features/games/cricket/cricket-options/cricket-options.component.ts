...
@Component({
  selector: 'app-cricket-options',
  templateUrl: './cricket-options.component.html',
  styleUrls: ['./cricket-options.component.css']
})
export class CricketOptionsComponent implements OnInit {
  ...
  constructor(//public router: Router,
    public dataService: CricketService, 
  ...
  getUsers(): void {
    this.userService.getUsers().subscribe({
      next: (response: UsersResponse) => {
        this.availablePlayers = response.users;
      },
      error: (error: any) => {
...
this.gameType = 'team';
  ...
  enableQuickStart(gameType : string) {
    ...
    this.playerOne = new CricketPlayer(9995,'Player1', '', 1);
    this.firstPlayerGame = new CricketGame(this.playerOne);
    this.inOrder = true
    this.userOne = {userId: 9995, firstName: 'Player 1', lastName: '', email: ''};
    
    if (gameType==='practice')
    {
      this.playerTwo = new CricketPlayer(9999,'CPU', '', 2);
      this.secondPlayerGame = new CricketGame(this.playerTwo);
      this.userTwo = {userId: 9999, firstName: 'CPU', lastName: '', email: ''};
    }
    else{
      this.playerTwo = new CricketPlayer(9996,'Player2', '', 2);
      this.secondPlayerGame = new CricketGame(this.playerTwo);
      this.userTwo = {userId: 9996, firstName: 'Player 2', lastName: '', email: ''};
    }
    console.log('playerTwo', this.playerTwo);
    console.log('userTwo', this.userTwo);
    this.enableStartGame(gameType);
